<template>
  <div class="yahe-page">
    <div class="page-header">
      ...
      <span>{{ lang === 'zh' ? '就医指南' : 'Medical Guide' }}</span>
    ...
    <div class="page-body">
      <div v-if="!activeItem">
        <div class="guide-item" v-for="item in guideItems" :key="item.id" @click="activeItem = item">
          ...
          <div class="guide-text">
            <span class="guide-title">{{ lang === 'zh' ? item.titleCn : item.titleEn }}</span>
            <span class="guide-sub">{{ lang === 'zh' ? item.subCn : item.subEn }}</span>
          </div>
      ...
      <div v-else>
        ...
        <div class="detail-title">{{ lang === 'zh' ? activeItem.titleCn : activeItem.titleEn }}</div>
        <div class="detail-content" v-html="lang === 'zh' ? activeItem.contentCn : activeItem.contentEn"></div>
      </div>
    </div>
  </div>
</template>
...
const guideItems = [
  {
    id: 1,
    titleCn: '国际医疗部介绍', titleEn: 'About Graceland',
    subCn: '中心简介、服务理念、专家团队', subEn: 'Introduction, philosophy, expert team',
...
<p>Over 200 top specialists with carefully selected resident medical staff provide outpatient, inpatient, premium health check and consultation services.</p>`
  ...
  {
    id: 2,
    titleCn: '就医导航', titleEn: 'Navigation',
    subCn: '门诊楼位置、停车指引、交通路线', subEn: 'Location, parking, transportation',
...
<p><strong>Metro:</strong> Line 5 Yuancun Station Exit B, ~8 min walk</p>`
  ...
  {
    id: 3,
    titleCn: '联系方式', titleEn: 'Contact',
    subCn: '服务热线、微信公众号、预约方式', subEn: 'Hotline, WeChat, appointment methods',
...
<p><strong>Appointment:</strong> Phone / WeChat / This mini-program</p>`
  ...
  {
    id: 4,
    titleCn: '就诊须知', titleEn: 'Visit Guidelines',
    subCn: '首诊流程、证件要求、注意事项', subEn: 'First visit process, ID requirements',
